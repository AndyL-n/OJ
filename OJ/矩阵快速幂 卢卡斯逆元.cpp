...
int cnt[100005];
void Merge_sort1(vector<Node>&vc,int l,int r)
{
	if(l>=r-1) return;
	int mid = (l+r)/2;
	Merge_sort1(vc,l,mid);
	Merge_sort1(vc,mid,r);
	int i=l,j=mid,t=0;
	Node *n = new Node[r-l];
	while(i<mid || j<r)
	{
		if(j>=r || i<mid && vc[i].v<=vc[j].v)
			n[t++] = vc[i++];
		else
		{
			cnt[vc[j].id] += mid-i;
...
void Merge_sort2(vector<Node>&vc,int l,int r)
{
	if(l>=r-1) return;
	int mid = (l+r)/2;
	Merge_sort2(vc,l,mid);
	Merge_sort2(vc,mid,r);
	Node *n = new Node[r-l];
	int i=mid-1,j=r-1,t=r-l-1;
	while(i>=l || j>=mid)
	{
		if(i<l || j>=mid && vc[i].v <= vc[j].v)
			n[t--] = vc[j--];
		else
		{
			cnt[vc[i].id] += j-mid+1;
...
int main()
{
	...
	vector<Node> uc(vc.begin(),vc.end());
	Merge_sort1(vc,0,vc.size());
	Merge_sort2(uc,0,uc.size());
	long long sum =0;
...
————————————————
版权声明：本文为CSDN博主「今天好好优秀了么」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/hhhhhh5863/article/details/89597658
